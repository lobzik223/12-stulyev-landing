  ...
  return (
    <section id="journey" className="py-20 bg-[#15110E] text-white">
      ...
      <div className="mt-8 overflow-x-auto px-4 md:px-8">
        <div className="flex gap-6 min-w-[700px] snap-x snap-mandatory">
          {items.map((c) => (
            <motion.article
              key={c.id}
              whileHover={{ y: -4 }}
              className="snap-start w-[320px] shrink-0 rounded-2xl overflow-hidden relative border border-[#D4AF37]/20 bg-black"
            >
              <div
